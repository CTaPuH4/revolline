<?php
/**
 * service.php — CDEK integration proxy
 * Добавлены CORS-заголовки и обработка preflight (OPTIONS) для локальной разработки.
 *
 * ВНИМАНИЕ: оставленные здесь значения аккаунта/ключа видны локально. Для продакшна
 * храните секреты в защищённом месте и не выкладывайте в публичные репозитории.
 */

/* ---------------------------
   CORS / preflight (в начало)
   --------------------------- */
// Белый список origin'ов для разработки / локалки.
// Добавьте сюда все адреса, с которых будет обращаться ваш фронтенд.
$allowedOrigins = [
    'http://127.0.0.1:5173', // Vite
    'http://localhost:5173',
    'http://127.0.0.1:3000', // CRA / dev
    'http://localhost:3000',
    'http://127.0.0.1:8000', // возможные локальные адреса
    'http://localhost:8000',
];

// Определяем пришедший Origin (если есть)
$origin = $_SERVER['HTTP_ORIGIN'] ?? '';

// Если origin в белом списке — возвращаем конкретный Origin (нужно для credentials)
// Иначе можно раскомментировать allow-all (только для отладки, без credentials)
if ($origin && in_array($origin, $allowedOrigins, true)) {
    header("Access-Control-Allow-Origin: $origin");
    header("Vary: Origin");
    // Если вы используете сессии / куки между фронтом и этим сервисом:
    header("Access-Control-Allow-Credentials: true");
} else {
    // Для быстрой локальной отладки можно разрешить всех:
...
/* Оригинальный код service.php (с минимальными правками для совместимости)
   — ниже создаётся объект service и вызывается process.
*/
...
class service
{
    ...
    private function sendValidationError($message)
    {
        ...
        // Удостоверимся, что Content-Type установлен
        header('Content-Type: application/json; charset=utf-8');
        // Сюда CORS заголовки уже добавлены в начале файла
